...
// The following lines related to BUTTON_PIN are now commented out as you are not using a physical button.
// #define BUTTON_PIN 4 
...
void sendStatus(String currentStatus) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Build URL with action parameter for the consolidated PHP handler
    String url = String(phpHandlerUrl) + "?action=update_status";
    Serial.print("[HTTP] Sending status to: ");
    Serial.println(url);

    if (http.begin(url)) { // Use the new consolidated URL
...
// This function is still present but not called from loop() if not using physical button.
// It remains here if you later decide to use another trigger mechanism from ESP32.
...
void setup() {
  ...
  // pinMode(BUTTON_PIN, INPUT_PULLUP); // COMMENTED OUT: Not using physical button
  // Serial.println("[SETUP] Button pin " + String(BUTTON_PIN) + " initialized as INPUT_PULLUP."); // COMMENTED OUT
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) { 
    ...
    // The following lines related to button state checking are commented out as you are not using a physical button.
